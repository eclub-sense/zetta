...
ServerResource.prototype.showServer = function(env, next) {
  ...
  // show only devices for the current user
  var userDevices = this._selectDevicesForLoggedUser(env, this.server.runtime._jsDevices);
  
  //TODO: argo-formatter may want to take multiple arguments for a format. This context obj is a hack.
  var context = { server: this.server, devices: userDevices, loader: this.getServer(env), env: env };
  env.format.render('server', context);
  next(env);
};

// Returns if the user owns the device (and therefore has full access).
// When no user is logged in, returns true.
// TODO that! 
ServerResource.prototype._loggedUserOwnsDevice = function(env, device) {
  if(env.auth && env.auth.username) {
    var authorizedDeviceKeys = env.auth.ownedDevices;
    
    if(authorizedDeviceKeys.indexOf(device.id) > -1) {
      return true;
    } else {
      return false;
    }
...
// Selects the devices according to the user logged in
ServerResource.prototype._selectDevicesForLoggedUser = function(env, devices) {
  var self = this;
  ...
  if(env.auth && env.auth.username) {
    ...
    keys.forEach(function(key, i) {
      ...
      if(self._loggedUserOwnsDevice(env, device)) {
...
ServerResource.prototype._queryDevices = function(env, next) {
  ...
  if(!params.ql) {
    ...
    var userDevices = self._selectDevicesForLoggedUser(env, self.server.runtime._jsDevices);
...
ServerResource.prototype.destroyDevice = function(env, next) {
  ...
  if(!device || !this._loggedUserOwnsDevice(env, device)) {
