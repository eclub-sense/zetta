...
RootResource.prototype.auth = function(env, next) {
  if(this.server.httpServer.zetta._useOAuth2) {
  var code = env.request.url.substring(15);
  //console.log(code);
  var token = {
    "code": code,
    "client_id": "michal",
  ...
  var options = {
    uri: 'http://zettor.sin.cvut.cz:8080/new_token',
    method: 'POST',
    json: token
  };
  ...
  request(options, 
    function(error, response, body) {
      if(!error && response.statusCode == 200) {
        //self.server.httpServer._accessToken = body.access_token;

        //@TODO CHECK IF HUB IS MINE!
        //- HAVE
        //Is hub mine?
        /*var checkUser = {
          uri: 'http://mlha-139.sin.cvut.cz:8080/registered_hubs',
          method: 'POST',
          qs: {access_token: body.access_token}
          path: {uuid: self.server._uuid}
        };

        request(options, function(error, response, body) {
          if(error || response.body === null) {
            env.response.statusCode = 400;
            env.response.body = "400 Unauthorised!";
            return next(env);
          }
        });*/

        //Redirect
        if(self.server.httpServer._lastURI) {
          var redirectURI = self.server._myIP+self.server.httpServer._lastURI;
        }else {
          var redirectURI = self.server._myIP;
        }
        //Append access token as parameter to redirection uri
        redirectURI += "?access_token="+body.access_token;
        //console.log(body.access_token)

        //env.response.setHeader('Authorization', body.access_token);
        env.response.writeHead(302, 'Found', { 'Location': redirectURI });
        env.response.end('');
