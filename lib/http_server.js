...
var oauth2 = require('simple-oauth2')({
  clientID: '443649814858-lvc1abj9ccnudm6l199f23ddqapgo1u3.apps.googleusercontent.com',
  clientSecret: 'Q4319mPrbfQIC0D8OF1ImBnP',
   ...
   .format({ engines: [Siren], override: { 'application/json': Siren }, directory: path.join(__dirname, './api_formats') })
    ...
    .use(function(handle) {
      handle('request', function(env, next) {
        ...
        //OAuth2
        if(self.zetta._useOAuth2) {

        //Check if token is in header
        if(env.request.headers['authorization']) {
          self._accessToken = env.request.headers['authorization'];
        }

        //Check if token is in parameter
        var query = require('url').parse(env.request.url,true).query;
        ...
        var authorization_uri = oauth2.authCode.authorizeURL({
          redirect_uri: 'https://zettor.sin.cvut.cz:8082/callback_hub',
          scope: 'email',
          state: self.zetta._myIP+"/callback"
        });

        //@TODO: Should implement token refreshing 
        //Check if access token is valid
          if(self._accessToken) {
            var options = {
              url: 'http://zettor.sin.cvut.cz:8080/registered_sensors',
              qs: {access_token: self._accessToken, access: "my"},
              method: 'GET'
            };

            request(options, function(error, response, body) {
              if(error || (response.statusCode != 200)) {
                //If token is not valid, delete it
                //@TODO REFRESHING!
                self._accessToken = undefined;

                //@TODO: Last URI per user
                if(query.access_token) {
                  self._lastURI = undefined;
                } else {
                  self._lastURI = env.request.url;
                }
                env.response.writeHead(302, 'Found', { 'Location': authorization_uri });
                env.response.end('');
              }else {
                self._accessToken = undefined;
            ...
            //No access token
            var requestURL = env.request.url;
            if(requestURL.slice(0, 9) == '/callback') {
              next(env);
            }else {
              //@TODO: Last URI per user 
              self._lastURI = env.request.url;
          ...
          //If we dont use oAuth2, then skip
          next(env);
        }
      });
    });
