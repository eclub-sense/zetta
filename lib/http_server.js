    ...
    .use(function(handle) {
      handle('request', function(env, next) {
        /*var authorization_uri = oauth2.authCode.authorizeURL({
          redirect_uri: 'https://mlha-139.sin.cvut.cz:8082/callback_hub',
          scope: 'email',
        ...
        //Check if access token is valid
          if(self._accessToken) {
            var options = {
              url: 'http://147.32.107.139:8080/registered_sensors',
              qs: {access_token: self._accessToken, hubID: self.zetta.id, access: "my"},
              method: 'GET'
            };
            ...
            request(options, function(error, response, body) {
              if(error || (response.statusCode != 200 && response.statusCode != 204)) {
                ...
                self._accessToken = undefined;
              	...
              	//console.log(self._accessToken);
              }
            });
          }

          var requestURL = env.request.url;
          if(requestURL.slice(0, 9) == '/callback' || self._accessToken) {
            self._accessToken = undefined;
            next(env);
          }else {
